...
export default function ScorePage({ route, navigation }) {
    ...
    const handleShare = async () => {
        ...
        const shareBadge = badgeMap[score].data64;
        const shareText = ` \n\n  I scored ${score} points on Quizzy News! 🎯\n https://www.quizzy.news`;

        try {
            // await Clipboard.setImageAsync(shareBadge);
            const htmlContent = `<img src="data:image/png;base64,${shareBadge}" alt="Share Badge" /> \n <p>${shareText}</p>`;
            ...
            alert('Badge Image Copied to clipboard and share text copied to clipboard');
        } catch (error) {
            console.log('Error copying badge image to clipboard:', error);
        };
    ...
    return (
        <View className="h-screen p-5 flex justify-center items-center bg-white ">
            ...
            {/* Score section - top 25%*/ }
            <View className="h-1/5 flex items-center justify-center relative width-full">
                {console.log(score)}

                <Svg width="146" className="absolute z-1 " height="146" viewBox="0 0 132 132" fill="none" xmlns="http://www.w3.org/2000/svg" >
                ...
                <Text className="text-sm font-lexend text-white z-10">Total Score</Text>
                </View>
            </View>
            {/* Question Review - middle 60% */}
            <View className="h-3/5 w-full pt-12 flex items-center width-full ">
            ...
            <View
                className="h-1/6 w-full px-5 pb-5 flex flex-row justify-between"
            >
